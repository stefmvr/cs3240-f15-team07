...
from .encryption import encrypt_body, secret_string
...
####################################################################################################################################
            ...
            if(Enc):
                pubkey = UserAttributes.objects.get(user_name = r.get_username()).publicKey
                pubKeyObj =  RSA.importKey(pubkey)
                thebody = self.instance.body
                str1 = ''.join(random.choice(string.ascii_uppercase) for i in range(12))
                self.instance.skk = (pubKeyObj.encrypt(str1.encode(), 32)[0])#.decode("cp037")#(secret_string("12345", pubKeyObj)[0])
...
class WriteForm(BaseWriteForm):
    ...
    class Meta(BaseWriteForm.Meta):
        fields = ('recipients', 'subject', 'body', 'encd')
...
##############################################################################################################
    class Meta(BaseWriteForm.Meta):
        fields = ('email', 'recipients', 'subject', 'body', 'encd')
#########################################################################################################
...
class FullReplyForm(BaseReplyForm):
    ...
    class Meta(BaseReplyForm.Meta):
        fields = (['recipients'] if allow_copies else []) + ['subject', 'body', 'encd']
