...
from .encryption import encrypt_body, secret_string, decrypt_body
import pickle
#---------------------------------------------------------------------------------------------------------------------------------------------------------------
...
###############################################################################################################################################
                #global THEKEY
                try:
                    THEKEY = kwargs['data']['pkey']
                    bod = items.mbod#kwargs['data']['emessage']

                    keyobj = RSA.importKey(THEKEY.encode())
                    str1 = items.mskk#kwargs['data']['eskk']
                    str2 = bytes(str1)
                    str3 = keyobj.decrypt(str2)
                    self.initial['body'] = decrypt_body(bod, str3)
                    #m.enc = False


                except:
                    self.initial['body'] = "ERROR"
...
####################################################################################################################################
    ...
    
    -----END RSA PRIVATE KEY-----"""


                keyobj = RSA.importKey(mykey.encode())
                str1 = m.skk
                ...
                str2 = bytes(str1)
                str3 = keyobj.decrypt(str2)
                m.body = decrypt_body(m.body, str3)
